...
- **Defog** (Shop consumable): clears the active field — Battlefield or Home Field — for the rest of combat. The guaranteed answer to a hostile field. (Implemented as "Defog", the canonical Pokémon field-clearer; this freed the "Smoke Ball" name, and the −20%-first-hit relic that previously held it was renamed to "Barrier Charm" — see §8.3.3. The survive-lethal effect is unaffected: it remains the separate Focus Sash Held Item, §8.4.4.)
