...
namespace ProjectAscendant.Combat
{
    ...
    public static class DamageCalculator
    {
        public static DamageBreakdown Compute(in MoveContext ctx)
        {
            ...
            int atk   = CombatStatResolver.EffectiveAttack(ctx.Attacker, ctx.Config);
            int def   = Mathf.Max(1, CombatStatResolver.EffectiveDefense(ctx.Target, ctx.Config));
        ...
        public static DamageBreakdown Preview(in MoveContext ctx) => Compute(ctx);
