...
{
    // Per §4.3.9.1 — the Pokedex's payoff: defeating a species enough times to reach the
    // Master tier (50 / 25 / 10 / 10 kills by rarity, tunable on PokedexProgressSO) permanently
    ...
    public static class PokedexMasteryUnlock
    {
        // Returns true iff this call newly unlocked the species' Mastery Move (caller should log it).
        public static bool TryUnlockMastery(PokedexProgressSO bestiary, MetaProgressionSO meta,
                                            PokemonSpeciesSO species)
        {
            ...
            if (bestiary.TierFor(species.SpeciesId) < PokedexMasteryTier.Master) return false;
