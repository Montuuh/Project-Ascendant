...
{
    // Per §7.5.1 (CL-024) — generic reusable "choose 1 of N relics" modal.
    // Extracted from StartingRelicPanelUI to support Elite Trainer Rare-relic choice (1 of 3).
    // Also used by StartingRelicPanelUI (thin wrapper) and Victory Road Gauntlet (§4.5.1.1).
    public sealed class RelicChoicePanelUI : MonoBehaviour
    {
        ...
        // Opens a relic choice modal with a custom title. Caller provides the offer list and a callback.
        // title: e.g., "CHOOSE A STARTING RELIC" (StartingRelic), "ELITE REWARD — CHOOSE 1 OF 3" (Elite Trainer).
        // offer: the relic options (typically 3).
        // onPicked: invoked with the chosen relic (or null if skipped).
        public void Open(IReadOnlyList<RelicSO> offer, string title, Action<RelicSO> onPicked)
        {
            _onPicked = onPicked;
            _font = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf");
            _root = new GameObject("RelicChoiceCanvas");
            ...
            Txt(_root.transform, title, 38, new Color(0.85f, 0.95f, 0.7f), new Vector2(0, 260), new Vector2(1300, 52));
