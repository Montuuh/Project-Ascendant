...
# Check the hardcoded swap cost (must be data-driven, not literal 1/2/3)
if grep -qE "swapCost\s*=\s*[123];" "$FILEPATH" 2>/dev/null; then
  echo "⚠️  HARDCODED SWAP COST in '$FILEPATH'."
  echo "    Swap cost should be calculated from SwapCounter, not hardcoded."
  echo "    Per §3.3.1: 1st swap=1AP, 2nd=2AP, 3rd=3AP."
  VIOLATIONS=$((VIOLATIONS+1))
fi

# IsFainted flag check — must not exist
if grep -qE "IsFainted|isFainted" "$FILEPATH" 2>/dev/null; then
  echo "❌  GDD VIOLATION in '$FILEPATH': IsFainted flag found."
  echo "    Per §2.4.1: 0 HP = Fainted. There is no separate IsFainted flag."
  VIOLATIONS=$((VIOLATIONS+1))
fi
